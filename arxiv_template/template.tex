...
\title{We need a nice title}
%title{Telling the story of best friends: marker rank statistic} -- старый. Но не ужасный.
...
\maketitle
\begin{abstract}
The analysis of the interaction of two different sets is essential across diverse fields, from linking drugs to side effects to associating genes with biological processes. However, not all interactions are equally informative. For example, a gene associated with a few specific biological processes provides more functional insight than a highly non-specific gene that acts within nearly every observed process. Identifying such specific interactions is challenging because they are often hidden among a large number of weak background connections. Moreover, the data might be aggregated from different sources, making it impossible to define universal identification criteria.

We introduce \textsf{friends.test}, a statistical method that formalizes specificity as a structural break in the distribution of an entity's interactions (e.g., a gene's associations to processes). A ranking-based representation ensures robustness to different data scales. To automatically locate the boundary between specific interactions and background activity, we employ model fitting.

\red{We demonstrate \textsf{friends.test} on transcriptomic data from head and neck squamous cell carcinoma, where it identifies a set of group-specific marker genes.} Using the detected interaction profiles, we further illustrate how they can be leveraged to construct a functional similarity space that reveals coherent gene modules. A computationally efficient \textsf{R} implementation of \textsf{friends.test} is available at \url{https://github.com/favorov/friends.test}.
...
\keywords{rank statistics \and feature selection \and specific gene regulation}
...
To address these challenges, we introduce \textsf{friends.test}: a computationally efficient, self-tuning approach for detecting specific interactions by identifying structural breaks in interaction profiles. To overcome the lack of a common scale, our method utilizes a rank-based representation, which normalizes disparate interaction strengths and ensures the procedure is scale-invariant. Furthermore, to ensure the method is adaptive, we employ model fitting. This allows the algorithm to automatically locate the structural break and determine an entity-specific threshold, effectively distinguishing meaningful signal from background noise for each entity individually.
...
To demonstrate the utility of the \textsf{friends.test}, we apply it to a transcriptomic dataset of head and neck squamous cell carcinoma (HNSCC). \textsf{R}-package is available at \url{https://github.com/favorov/friends.test}. The package 
 runs in $\mathcal{O}(nk\log(n))$ times, where $n$ is number of rows, $k$ is number of columns. That performance makes the package scalable for large matrices. 
...
The first approach is the \textit{pre-fitting uniformity test}. That is, we assess whether items in  $\mathrm{row}_i(R)$ are uniformly distributed before estimating the mixture parameters.
...
\SetAlgoLined
\DontPrintSemicolon
\KwIn{Interaction matrix $A$, entity index $i$, prior probability of having friends $q$ or significance level $\alpha$, \\testing mode $\in \{\text{Test, IC}\}$}
\KwOut{Estimated set of friends $\hat{F}$}

\BlankLine
\tcp{Step 1: Normalization}
Convert interaction matrix $A$ to rank matrix $R$ using randomized tie-breaking\;
Extract $\mathrm{row}(R)_i = (r_1, \dots, r_k)$\;
Estimate support boundaries $\hat{u} = \min(r_j)$ and $\hat{w} = \max(r_j)$\;

\BlankLine
\tcp{Step 2: Uniformity test}
\If{mode == \text{Test}}{
    Perform a uniformity test on $\mathrm{row}_i(R)$\;
    \If{$p\text{-value} > \alpha$}{
        \Return{$\emptyset$ \tcp*[f]{No friendship detected}}
    }
}

\BlankLine
\tcp{Step 3: Model Fitting}
$L_{max} \leftarrow -\infty$\;
\For{$m \in \{r_{(1)}, \dots, r_{(k-1)}\}$}{
    $s \leftarrow \text{count}(\{r_j \le m\})$\;
    $p \leftarrow s/k$\;
    $L_{curr} \leftarrow s\ln\left(\frac{p}{m-\hat{u}+1}\right) + (k-s)\ln\left(\frac{1-p}{\hat{w}-m}\right)$\;
    \If{$L_{curr} > L_{max}$}{
...
\BlankLine
\tcp{Step 4: Information criterion}
\If{mode == \text{IC}}{
...
\Return{$\hat{F} = \{c_j : r_j \le \hat{m}\}$}
\caption{The \textsf{friends.test} procedure}
\label{alg:friends}
\end{algorithm}


\section{Experimnetal results}
...
%Similarity analysis of cancer marker genes, based on their sets of friends, highlighted functional structure of the gene set. 
...
Finally, widely used specificity indices, such as the Tau index or the Gini coefficient, quantify how unevenly values are distributed within an interaction profile, thereby providing a single-number measure of overall specificity. However, these metrics do not identify which particular interactions constitute the specific signal \cite{kryuchkova2017benchmark}.

%разобраться с Gini и likelihood



% Unlike Gene Set Enrichment Analysis (GSEA), which relies on a priori knowledge of groups, the \texttt{friends.test} is fully unsupervised, discovering specific connectivity patterns de novo using rank statistics to ensure robustness against distributional heterogeneity.
...
4 & \genecard{C16orf73} & Single-stranded DNA-binding; meiosis & \red{No established cancer role} \url{https://www.nature.com/articles/ncomms10499} \\ \hline
5 & \genecard{C1orf110} & Coiled-coil protein; cell cycle/DNA repair & \red{No clear cancer role} Downregulated gene in HNSC \url{https://tcr.amegroups.org/article/view/45134/html} \\ \hline
