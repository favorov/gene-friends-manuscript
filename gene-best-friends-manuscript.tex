...
The ranking method can be applied in a na\"ive way: the higher the attention of a collection to a tag is, the more friendly the collection is to the tag. But this approach cannot tell between the tags that are specific markers and the tags which are important for all collections (e.g., so-called network hubs).

Therefore, we suggest using double-ranking. It points to the most friendly collection for a given tag. 

First, for each collection $c_j$ we decreasingly rank the attention values $a_{1j}, \dots, a_{nj}$. As a result, we get the ranks $r_{1j}, \dots, r_{nj}$.  The lower $r_{ij}$ is, the higher the importance of the tag $t_i$ for the collection $c_j$. See Section~\ref{sec:method} for details.
