...
%
\author{
Alexandra Suvorikova \inst{1} \and Vasily Ramensky \inst{2,3,5} \and Vera Mukhina \inst{4} 
\and Andrey Mironov \inst{5} \and Alexander Favorov\inst{6}\textsuperscript,\inst{7}}
%
\authorrunning{A. Suvorikova et al.} % abbreviated author list (for running head)
%
%%%% list of authors for the TOC (use if author list has to be modified)
\tocauthor{Alexandra Suvorikova, Vasily Ramensky, Vera Mukhina, Andrey Mironov, and Alexander Favorov}
%
\institute{
Weierstrass Institute, \\ Berlin, Germany, 
\email{suvorikova@wias-berlin.de}
\and
MSU Institute for Artificial Intelligence, Lomonosov Moscow State University, \\ Moscow, 119992, Russia
\and
National Medical Research Center for Therapy and Preventive Medicine of the Ministry of Healthcare of Russian Federation, \\ Moscow, 101990, Russia,
\and
...
\begin{abstract}
The weighted bipartite graph model fits many practical problems, such as gene expression regulation by a set of transcription factors. %or weights of genes in expression patterns. 
...
\keywords{specific gene regulation, pattern marker, weighted bipartite graph, noteworthy edges, rank statistics, feature selection, clustering, knowledge transfer}
...
% % . \textcolor{red}{ \textit{Actually, we say nothing about friendship afterward. To rewrite the sentence?}}
% Let's consider several examples. The first one comes from information retrieval. The goal is to find the documents that are most relevant to the user's query among all available documents \cite{liu2009learning}. %friendship.

% In the second example, we consider a set of genes and their weights in a set of expression patterns \cite{fertig_cogaps_2010, Fertig_2016,stein-obrien_enter_2018}. Each pattern represents a biological process by the expression levels of the involved genes, and we know all the weights of genes in patterns.
...
% To avoid mistakes in marker selection, we develop a method that looks for features that are more important for particular patterns than for others, and for these patterns at the same time. The patterns are referred to as friends of their marker feature. 

%Мы хотим видеть пару маркер - друг не только тогда, когда у маркера самый большой вес внутри процеесса, мы хотим видеть, когда у маркера самый большой вес в процессе, если процессы сравнить между собой. Это даёт возможность убирать универсальные маркеры (хабы). Тут наверное можно сказать что если ген - маркер, то процесс - его друг (лучший друг)

% % Factorization results in an approximate matrix product decomposition. One of the factors is an \textit{amplitude} matrix that describes the structure of complex biological processes in terms of weights of the features (e.g., gene expressions) in the processes. 

% \textcolor{purple}{Sometimes, the expression of a single gene indicates the activity of the entire pattern. In the simplest case, the gene has a nonzero load in only one pattern. In a more realistic setting, the gene has several nonzero loads, but all of them except one are relatively small. Then the gene (AKA Augustus) is the marker gene \cite{stein-obrien_patternmarkers_2017} for the pattern, and the marked pattern is the best friend of this gene. We want to identify the statistically significant marker genes and corresponding patterns.}

% The third example deals with gene-to-gene correlation matrix (e.g. correlation of expression). High correlation indicates high similarity between genes. For a given gene, the goal is to indicate its ``best-friend'' genes among genes. 
% % %friendship.
...
\begin{tabular}[c]{@{}c@{}}gene expression regulation\\  by TFs\end{tabular}     & gene             & \begin{tabular}[c]{@{}c@{}}genes under \\ TF regulation\end{tabular}     & \begin{tabular}[c]{@{}c@{}}strength of \\ regulation\end{tabular}       \\ \hline
...
Partial or total ranking of documents for a given query is a popular strategy in information retrieval \cite{liu2009learning}. In bioinformatics, many methods rely on ranking, e.g. feature selection \cite{venetis2011selection}, marker selection \cite{vargo2020rank,pullin2022comparison,jimenez2021multivariate}, etc.

The idea of ranking can be applied in a na\"ive way: the higher the attention of a collection to a tag is, the more friendly the collection is to the tag. But this ranking cannot tell between the tags that are specific markers, and the tags which are common features for all collections (e.g., gene hubs).
...
% to define the concept of friendship via the concept of care in accordance with the Augustus intuition, and thus 
...
Here, the tag splits the set of all collections into two subsets, e.g. the friends of the tag and all the others. The split occurs by a chosen threshold for ordered ranks. The \test{friends test} estimates the statistical significance of this split.
...
The software that implements the \text{best friend test} and \text{friends test} is currently available as an \textsf{R} package \textsf{best.friends} at\\
