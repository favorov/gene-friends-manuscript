...
% There are three main strategies for dealing with this problem
% \begin{itemize}
% \item To specify whether a document is relevant the query
% (using binary judgment or judgment with a certain degree). Then for a fixed query, directly compare the relevance of two documents.
% \item To estimate the relative relevance between documents for a given query.
% \item To establish the documents' partial (or total) order for a query.
% \end{itemize}



%\textcolor{airforceblue}{RANKING -- The method is distribution free.  Ranking appears in many applications, such as relevant feature selection \cite{venetis2011selection}, marker selection \cite{vargo2020rank}, \cite{pullin2022comparison}, information retrieval \cite{jimenez2021multivariate}, etc. Ranking relaxes the same-nature and same-distribution assumptions.} 

%\textcolor{green}{A na\"ive approach is to rank all the genes by the correlation with our gene. However, the ranking cannot tell between specifically correlated genes and network hub genes (i.e. with genes that carelessly correlate with all the other genes)}. 


%We suggest using double-ranking approach. It is distribution-free and can distiguish between \textcolor{red}{friends and true friends}.
...
% \textcolor{purple}{For each tag, we want to identify the collection(s) that particularly prefer(s) the tag, if any. Then such a collection is a friend (or the best friend if there is only one) for the tag. The simplest example: imagine that only one collection pays attention to the tag in hand. }

% \textcolor{airforceblue}{ 
% Partial or total ranking of documents 
% In the information retrieval a popular solution is to establish the documents' partial or total order for a query, and then to pick up the most relevant \cite{liu2009learning}.}

% \textcolor{airforceblue}{Ranking approach is distribution-free. In other words, it relaxes modelling assumptions if there are any. However, in some cases it is not enough.}
...
%For a similar test that splits all the collections into $m$ friends of the tag and the remaining $|C|-k$ collections uses the difference  $r(t_i,c_{(m+1)}(t_i))$ and $r(t_i,c_{(m)}(t_i))$. If we obtain the p-value that is small enough, we claim that the collections $c_{(1)}(t_i)$..$c_{(m)}(t_i)$ are friends of $t_i$ and $t_i$ is their marker.

\textcolor{purple}{Marker}

% \textcolor{yellow}{We have vector of ranks of the tag in different collections. We can say whether the best is the best friend (AKA real best friend). We also can say it about each split}
...
% Sometimes, the attention matrix for a set of documents is represented as a "tag cloud" \cite{venetis2011selection}. Tags in such a cloud can be shown as hyperlinks to the subset of the documents related to the tag.
% In the case of information retrieval, the collections represent similar objects and thus can be naturally compared with each other by a tag's relevance \cite{liu2009learning}. 


% This might be not the case for clouds being biological processes.
...
% Now, the question. For each tag, we want to identify the collection(s) that specifically prefer(s) the tag. We say that such a collection is a friend (or the best friend if it is the only one) for the tag. The simplest example: imagine that only one collection pays attention to our tag. 
