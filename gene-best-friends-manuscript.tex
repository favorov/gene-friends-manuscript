...
The second example deals with matrix factorization of a multisample and, possibly multimodal, omics matrix \cite{fertig_cogaps_2010, Fertig_2016,stein-obrien_enter_2018}. 
The factorization results in an approximate  matrix product decomposition. One of the factors is an \textit{amplitude} matrix describing the structure of complex biological processes in terms of weights of the features (e.g., gene expressions) in the processes. 
...
All the values $a_{ij}$ are stored in a matrix $\mathcal{A}$ \textcolor{red}{(one may think of non-diagonal block a weighted adjacency matrix of the corresponding graph)}; its row $\mathcal{A}_{i:}$ corresponds to tag $t_i$, its column $\mathcal{A}_{:j}$ corresponds to collection $c_j$.
