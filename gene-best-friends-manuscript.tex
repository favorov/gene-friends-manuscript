...
The tag-cloud-attention model applies to many problems in bio-informatics and statistics.
Tab.\ref{tab:examples} contains some of them.
\begin{table}[h!]
\centering
\begin{tabular}{c|c|c|c}
\textbf{Example} & \textbf{tag $t$} & \textbf{cloud $c$} & \textbf{attention $A(t, c)$} \\ 
\hline
...
%
\section{Method}
The attention matrix $\mathcal{A}$ plays a key role in the statistical test. Recall its elements \textcolor{red}{are the strength of attention} $A(t_i, c_j)$ the cloud $c_j \in C$
 pays to the tag $t_i \in T$. For the sake of simplicity, we will write $a_{ij}$ instead of $A(t_i, c_j)$. Thus, we get
\[
...
In other words, each column in $\mathcal{A}$ stores the information about the strength of attention that the corresponding cloud pays to all tags in $T$. We denote the \textcolor{red}{respective} column as 
$\mathcal{A}_{:j} =(a_{1j}, \dots a_{nj})$ for all clouds $c_j$, $j  \in \{1, \dots, k\}$.

Under $H_0$ we assume all items in $\mathcal{A}$ are independent. Moreover, the items in
...
$\mathcal{A}_{:j}$.
Namely, we decreasingly order $\mathcal{A}_{:j}$, and find the ordinal number for each $a_{ij} \in \mathcal{A}_{:j}$,
$R_{ij} := \text{rank}(a_{ij}|\text{inside}~\mathcal{A}_{:j})$. We resolve ties by the mean rank of the tied elements.
...
\]
Each item in $\mathcal{R}$ is the normalized attention's rank the cloud $c_j \in C$ pays to the tag $t_i \in T$. Let $\mathcal{R}_{i:}$ be the $i$-th row in $\mathcal{R}$.

\textcolor{red}{In order to find the most friendly cloud for the tag $t_i\in T$} we rank the elements inside
$\mathcal{R}_{i:}$. 
...
To assess whether the most friendly cloud is the best friend, we estimate the difference 
\[
u(t_i) := c_{(1)}(t_i) -  c_{(2)}(t_i).
\]
