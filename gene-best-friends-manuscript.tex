...
\subsection*{\textcolor{red}{A great phrase!!!}}
In this manuscript, we develop a method to detect noteworthy edges in a weighted bipartite graph. We suppose all edges of the graph to be co-directed. The graph models a directed relation (referred to as attention) from the vertices of one part (collections) to the vertices of  another part (tags).
...
This model describes a wide spectrum of problems, where one is interested in detecting specific markers characterizing a process or a data set in hand.


In this manuscript, we suggest a novel method of analyzing the bipartite graph modeling tag-collection relationship. 


Namely, we formulated a statistical test that detects specific markers (tags) that characterizes a given collection.  

The proposed model covers numerous straightforward applications. 
...
analyzing the relationships between two partitions of 

In this manuscript, we numerically express the concept of friendship. We start with a matrix of the values of the attention paid by $k$ collections to $n$ tags, then 


We introduced a statistical procedure test that validates the hypothesis of 
...
to detest reliable pairs of marker features and the marked entity. Nevertheless, the formulation of the problem looks abstract, its solution provides numerous straightforward applications. 
\subsection*{\textcolor{red}{End of great phrase!!!}}
