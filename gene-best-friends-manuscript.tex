...
%
\author{
Alexandra Suvorikova \inst{1} \and Vasily Ramensky \inst{2,3,4} \and Vera Mukhina \inst{5,6} 
\and Andrey Mironov \inst{4,7} \and Alexander Favorov\inst{6,8}}
%
...
There is a simple intuition of what does it mean to be a friend. Augustus is more important to his friend than to other people. And if we see Augustus, then we also infer that we will see friend(s) of Augustus. 

In the following, we formulate this concept as a double ranking procedure and a statistical test that validates its result to create a multi-purpose numeric method that identifies highly specific interactions.
...
Of note, neither a noise feature nor a feature with large weights in all processes should be identified as a marker. Here, to achieve this, we redefine markers as features that are more important for a particular process than for others. The friend is a complementary concept to this definition of a marker. If a process is a friend to a feature, the feature is a marker to the process, and vice versa. To properly identify pairs friend-marker pairs,  we suggest comparing the weights of all features in all processes simultaneously.  
...
\begin{tabular}[c]{@{}c@{}}gene expression regulation\\  by transcription factors\end{tabular}     & gene             & \begin{tabular}[c]{@{}c@{}}genes under the  transcription \\ factor regulation\end{tabular}     & \begin{tabular}[c]{@{}c@{}}strength of \\ regulation\end{tabular}       \\ \hline
...
The ranking method can be applied in a na\"ive way: the higher the attention of a collection to a tag is, the more friendly the collection is to the tag. But this approach cannot tell between the tags that are specific markers and the tags which are important for all collections (e.g., gene hubs).

Therefore, we suggest using double-ranking. It points to the most friendly collection for a given tag. 

First, for each collection $c_j$ we decreasingly rank the attention values $a_{1j}, \dots, a_{nj}$. As a result, we get the ranks $r_{1j}, \dots, r_{nj}$.  The lower $r_{ij}$ is, the higher the importance of the tag $t_i$ for the collection $c_j$. 
