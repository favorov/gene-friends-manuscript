...
%=========
% Let's say we have an $A_{ij} = A(\tl_i, \cl_j)$ - matrix that represent the strength of relation between $k = |T|$ tags and $n = |C|$ clouds. We want to identify all entities that are preferably selected (marked) by one or more features. Such a feature is a marker for the entity, and the entity is the best friend for the feature.
% First, we rank all the features by its $A_{ij}$ for each entity $j$, so obtaining $R_{ij}= \textrm{rank of } P_{ij} \textrm{ in } P_{*j}$. Then, we scale $R$ into $[0..1]$ : $r_{ij}={R_{ij}}/{k}$. To be specific, we suppose that the ranking is ascending, the smaller is the rank, the stronger is the relation.
Let $T = \{t_1, \dots, t_n\}$ be a set (source pool) of tags, and set of \textit{clouds} $C = \{c_1, \dots, c_k\}$. Each cloud is a set of tags coming from $T$. %W.l.g. we assume $|T| = k$, and $|C| = n$.
...
%\textcolor{red}{Now we are able} to identify the most friendly cloud $\cl_{(1)}(\tl_i)$ for a tag $\tl_i$.
% Let $R_{i} = (r_{i1}, \dots, r_{ik})$ be the $i$-th row of matrix $R$ which corresponds to the \tag $\tl_i$. We sort $R_i$ in the decreasing order and find the largest element,
% $$
% j^* = 
% \underset{j}{\text{argmax}}~\text{rank}(r_{ij}|\text{inside}~R_i),
% \quad
% c_{(1)}(t_i) = c_{j^*}. 
% $$

% \[ \text(find)
% q = \text{rank}(r_{ij}|\text{inside}~R_i),
% \quad
% c_{(q)}(t_i)
% \]
