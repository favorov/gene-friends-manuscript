...
%
\section{Method}
The attention matrix $\mathcal{A}$ plays a key role in the statistical test. Recall its elements are 
$A(t_i, c_j)$ for all $t_i \in T$ and $c_j \in C$. For the sake of simplicity, we will write $a_{ij}$ instead of $A(t_i, c_j)$. For all $i \in \{1, \dots, n\}, j  \in \{1, \dots, k\}$ $a_{ij} = A(\tl_i, \cl_j)$ and
\[
...
Thus, each column in $\mathcal{A}$ stores the information about the strength of attention that the corresponding cloud pays to all tags in $T$. We denote the corresponding column as 
...
Now, for each tag, we define the degree of the friendliness of a cloud for this tag, by ranking the tag's row $\mathcal{R}_{i:}= \{r_{i1}, \dots, r_{ik}\}$. The most friendly cloud $c_{(1)}(t_i)$ is the cloud with the minimal value of $R_{ij}$, the next is $c_{(2)}(t_i)$, etc, etc. 
\[
c_{(q)}(t_i) = c_{j}, \quad \text{where}~j~\text{is such that}~ q = \text{rank}(r_{ij}|\text{inside}~\mathcal{R}_{i:}).
\]

Under $H_0$ we assume that all items in $\mathcal{A}$ are independent, and
$\mathcal{A}_{:j}$ are i.i.d inside each cloud $c_j$.
\textcolor{red}{Symmetric case?}
