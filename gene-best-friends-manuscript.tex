...
We express how much a collection $c_j$ cares about a tag $t_i$ in terms of the rank of $a_{ij}$ in column $\mathcal{A}_{:j}$. So, if a collection $c_j$ cares about a tag $t_i$ more than other collections, the rank of $a_{ij}$ in $\mathcal{A}_{:j}$ is higher than the rank of $a_{il}$ in $\mathcal{A}_{:l}$ for all $l \neq j$. 
...
% Under $H_0$ we assume that all the collections do not tell tags from each other.
% In other words, all the $n \times k$ attention values from collections to tags are independent (all $a_{ij}$ are independent). Moreover, all the $n$ attention values that a collection pays to tags are identically distributed (all $a_{ij} \in \mathcal{A}_{:j}$ are i.i.d).
